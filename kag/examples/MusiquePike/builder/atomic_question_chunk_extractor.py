...
@DecomposerABC.register("atomic_question_chunk_extractor")
class AtomicQuestionChunkExtractor(DecomposerABC):
    """
    ...
    def atomic_question_decomposition_from_chunk(self, passage: str):
        """
        ...
        """
        atomic_question_list = self.llm.invoke(
            {"content": passage}, self.decomposition_prompt, with_except=False,with_json_parse=False
        )

        return atomic_question_list
    ...
    def assemble_sub_graph_with_atomic_questions(
        self, sub_graph: SubGraph, atomic_questions: List[Dict]
    ):
        """
    ...
    def _invoke(self, input: Input, **kwargs) -> List[Output]:
        ...
        passage = title + "\n" + input.content
        out = []
        atomic_questions = self.atomic_question_decomposition_from_chunk(passage)
