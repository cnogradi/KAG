...
@PromptABC.register("baml_lf_static_planning")
class RetrieverLFStaticPlanningPrompt(PromptABC):
    ...
    default_case_en = [
        {
            ...
            "plans": [
                {
                "idea" : "First, retrieve the sports teams Cristiano Ronaldo played for in 2011",
                "step" : "Which Sports Teams Cristiano Ronaldo Played for in 2011?",
                "action" : "Retrieval(s=s1:Player[`Cristiano Ronaldo`],p=p1:PlayedForIn2011Year,o=o1:SportsTeam)"
                },
                {
                "idea" : "Next, obtain the foundation years of these teams",
                "step" : "What are the foundation years of the teams retrieved in plan[0]?",
                "action" : "Retrieval(s=o1,p=p2:FoundationYear,o=o2:Year)"
                },
                {
                "idea" : "Finally, determine the team founded last by comparing years",
                "step" : "Which team has the most recent foundation year based on plan[1]'s results?",
                "action" : "Math(content=[`o2`], target=`Which team was founded last?`)->math3"
                }
            ]
        },
        {
            ...
            "plans": [
                { 
                "idea" : "First, infer the relationship between James and Mike",
                "step" : "What is the familial relationship between James and Mike?",
                "action" : "Deduce(op=entailment,content=[`John is Mike's father`, `James is John's father`],target=`What is the relationship between James and Mike?`)->res"
                },
                {
                "idea" : "Output the deduced result",
                "step" : "What is the final answer from the relationship deduction by plan[0]?",
                "action" : "output(res)"
                }
            ]
        },
        {
            ...
            "plans": [
                {
                "idea" : "First, identify the association publishing the Journal of Psychotherapy Integration",
                "step" : "Which association publishes the Journal of Psychotherapy Integration?",
                "action" : "Retrieval(s=s1:Player[`Psychotherapy Integration`],p=p1:Publish,o=o1:Association)"
                },
                {
                "idea" : "Next, retrieve the first president of this association",
                "step" : "Who was the first president of the association retrieved in plan[0]?",
                "action" : "Retrieval(s=o1,p=p2:FirstPresident,o=o2:Person)"
                }
            ]
        },
        {
            ...
            "plans": [
                {
                "idea" : "First, locate the state where *Pocahontas Mounds* is situated",
                "step" : "Which State Where Pocahontas Mounds is Located ?",
                "action" : "Retrieval(s=s1:HistoricalSite[`Pocahontas Mounds`], p=p1:LocatedIn, o=o1:State)"
                },
                {
                "idea" : "Next, retrieve the state's admission year to the U.S.",
                "step" : "What year was the state retrieved in plan[0] admitted to the U.S.?",
                "action" : "Retrieval(s=o1, p=p2:StatehoodYear, o=o2:Date)"
                }
            ]
        },
        {
            ...
            "plans": [
                {
                "idea" : "First, identify the two tornado outbreaks",
                "step" : "Which is the first tornado outbreaks?",
                "action" : "Retrieval(s=s1:Event[`Tornado Outbreak`], p=p1:TheFirst, o=o1:Event)"
                },
                {
                "idea" : "Confirm the second event details",
                "step" : "Which is the second tornado outbreaks?",
                "action" : "Retrieval(s=s2:Event[`Tornado Outbreak`], p=p2:TheSecond, o=o2:Event)"
                },
                {
                "idea" : "Retrieve casualties from first outbreak",
                "step" : "How many people died in the first tornado outbreak retrieved in plans[0]?",
                "action" : "Retrieval(s=s1, p=p3:KilledPeopleNumber, o=o3:Number)"
                },
                {
                "idea" : "Retrieve casualties from second outbreak",
                "step" : "How many people died in the second tornado outbreak retrieved in plans[1]?",
                "action" : "Retrieval(s=s2, p=p4:KilledPeopleNumber, o=o4:Number)"
                },
                {
                "idea" : "Compare fatality numbers between events",
                "step" : "To compare the death toll between two tornado outbreaks to determine which one had more fatalities.",
                "action" : "Math(content[`o3`,`o4`], target=`Which one had more fatalities?`)->math5"
                }
            ]
        },
        {
            ...
            "plans": [
                {
                "idea" : "First, retrieve *Aas Ka Panchhi*'s release date",
                "step" : "When was Aas Ka Panchhi released?",
                "action" : "Retrieval(s=s1:Work[`Aas Ka Panchhi`], p=p1:ReleaseTime, o=o1:Date)"
                },
                {
                "idea" : "Next, retrieve *Phoolwari*'s release information",
                "step" : "When was Phoolwari released?",
                "action" : "Retrieval(s=s2:Work[`Phoolwari`], p=p2:ReleaseTime, o=o2:Date)"
                },
                {
                "idea" : "Compare release dates to determine earliest",
                "step" : "Comparing the release dates of Aas Ka Panchi and Phoolwari, who came earlier based on results retrieved in plam[0] and plan[1]",
                "action" : "Math(content=[`o1`,`o2`], target=`Comparing the release dates of Aas Ka Panchi and Phoolwari, who came earlier?`)->math5"
                }
            ]
        }
    ]
    ...
    def __init__(self, std_schema: StdSchema = None, **kwargs):
        self.template_zh = "lspp"
        self.template_en = "lspp"
    ...
    def parse_response(self, response: str, **kwargs):
        try:
            response = b.LFStaticPlanningPrompt(get_now(language='en'), json.dumps(self.default_case_en, ensure_ascii=False, indent=2), kwargs['query'])
            logger.debug(f"logic form:{response}")
        except Exception as e:
            logger.warning(f"{response} parse lf static planning prompt faied {e}", exc_info=True)
            return []

        logic_forms = self._parse_lf([ plan.step for plan in response.plans], [ plan.action for plan in response.plans])
        tasks_dep = {}
